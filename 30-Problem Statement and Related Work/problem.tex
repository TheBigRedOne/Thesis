...
This chapter formalises the producer mobility problem studied in this thesis. We first define the network and mobility model and then state the baseline behaviour that causes long service disruption under producer movement. Finally, we specify the evaluation goals and metrics used throughout the remainder of the dissertation.

%==============================================================================

\section{System Model}
\label{sec:problem:system_model}
...
\begin{itemize}
    \item \textbf{Producer mobility.} A \emph{producer} serves a name prefix (e.g., a live stream prefix) and may move between attachment points (edge routers). A \emph{mobility event} occurs when the producer detaches from one access router and reattaches to another, potentially changing the forwarding path from consumers to the producer.
    \item \textbf{Micro-mobility scope.} The primary scope of this thesis is \emph{intra-domain} producer mobility (micro-mobility), where the producer remains within the same administrative domain and the naming scheme stays stable. Consumers are assumed to continue to request the same name prefix across mobility events.
    \item \textbf{Traffic pattern.} We focus on continuous delivery workloads (e.g., live streaming or repetitive chunk retrieval) where consumers generate Interests at a steady pace and the producer responds with a steady stream of Data. This traffic pattern stresses continuity: even short disruptions can lead to visible degradation at the application layer.
\end{itemize}

%==============================================================================

\section{Baseline Mobility Behaviours in NDN}
...
In a link-state control plane, convergence requires (i) neighbour / adjacency change detection, (ii) dissemination of updated routing advertisements, and (iii) recomputation and installation of new routes. These steps are periodic and timer-driven in practice, so convergence can be substantially slower than the underlying data plane forwarding timescale. Consequently, \emph{service disruption after producer movement is dominated by a routing convergence delay rather than a packet transmission delay}.

%==============================================================================

\section{Problem statement}
\label{sec:problem:problem_statement}

We study the following problem.

\begin{quote}
    \textbf{Producer mobility continuity problem.} Given a producer that changes its attachment point within a domain, how can the network maintain (or rapidly restore) Interest--Data exchange for ongoing consumers \emph{without waiting for global routing convergence}, while keeping overhead and safety risks bounded?
\end{quote}
...
\begin{enumerate}
  ...
  \item \textbf{Safety:} ensure that the recovery mechanism cannot be trivially abused to cause persistent flooding, routing corruption, or unbounded state growth.
\end{enumerate}
...
\begin{figure}
    ...
    \label{fig:NDN_Producer_Mobility_Problem}
\end{figure}

%==============================================================================

\section{Assumptions and Design constraints}
\label{sec:problem:assumptions_and_design_constraints}
...
\begin{itemize}
  \item \textbf{Point-to-point forwarding scope.} Flooding-based recovery, if used, is restricted to point-to-point faces and excludes local/control faces to avoid accidental amplification through local-only channels.
  ...
  \item \textbf{Temporary forwarding state.} Fast recovery may rely on short-lived forwarding hints/state (e.g., a temporary FIB) that expires quickly (second-scale) and is refreshed only when used, preventing indefinite persistence. Our prototype uses a temporary FIB (TFIB) with an idle expiry on the order of 5 \,s and a sliding refresh on use. 
...
\label{sec:problem:evaluation_goals}

The thesis evaluates mechanisms against the following goals:
...
We define a mobility event time as $t_m$. Let $t_s$ be the time when the consumer resumes receiving Data \emph{continuously} (i.e., sustained delivery rather than a single sporadic Data). Using these, we define the following.

\subsection{Continuity and performance}
\label{sec:problem:metrics:continuity_and_performance}
...
\label{sec:problem:metrics:state_and_safety_indicators}

\textbf{Temporary state footprint.} We measure the number of temporary forwarding entries (e.g., TFIB size over time) and their lifetimes. A desirable property is fast decay after the mobility transient; in our prototype, TFIB is explicitly short-lived (second-scale) and refreshed only by use. 

\textbf{Duplication effectiveness.} We report the observed rate of duplicates suppressed by per-packet identifiers/caches (for both Data and Interest flooding), since duplication directly drives overhead and potential instability. 
